...
class BarChartHorizontalWithLabels extends React.PureComponent {
    ...
    componentWillMount = async () => {
        ...
        this.setState({ week1taks: parseInt(week1taks.data[0].TarefasConcluidas) })
        const week2taks = await axios.get(`${server}/tasks/week?dateI=${week2i}&dateF=${week2e}`)
        this.setState({ week2taks: parseInt(week2taks.data[0].TarefasConcluidas) })
        const week3taks = await axios.get(`${server}/tasks/week?dateI=${week3i}&dateF=${week3e}`)
        this.setState({ week3taks: parseInt(week3taks.data[0].TarefasConcluidas) })
        const week4taks = await axios.get(`${server}/tasks/week?dateI=${week4i}&dateF=${week4e}`)
        this.setState({ week4taks: paserInt(week4taks.data[0].TarefasConcluidas) })
    }
    ...
    render() {
        ...
        const CUT_OFF =  (Math.max(this.state.week1taks, this.state.week2taks, this.state.week3taks, this.state.week4taks)) - 1
